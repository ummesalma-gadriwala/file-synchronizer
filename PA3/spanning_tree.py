...
def diameter (nodes, G):
    """
    YOUR CODE:
    Calculate and return the diameter of input graph.
    """
    return Diameter
...
def check_spanning_tree(nodes, G):
    """
    YOUR CODE:
    Check if the input graph is spanning tree or not!
    """
...
def _calc_mdst ():
  """
  Calculates the minimum diameter spanning tree
...
def _calc_spanning_tree ():
  """
  Calculates the first found spanning tree
...
# If True, calculate the minimum diameter spanning tree
_mdst = False
...
def launch (no_flood = False, hold_down = False, mdst=False):
  global _noflood_by_default, _hold_down, _mdst
  if no_flood is True:
